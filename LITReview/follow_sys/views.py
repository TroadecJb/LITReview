...
class Subscriptions(generic.ListView):
    ...
    def get(self, request):
        ...
        subscribers = [
            sub.user
            for sub in UserFollows.objects.filter(followed_user=current_user)
        ]
    ...
    def post(self, request):
        ...
        if (
            User.objects.filter(username=followed_user_username).exists()
            and followed_user_username != request.user.username
        ):
